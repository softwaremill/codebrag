...
import com.softwaremill.codebrag.domain.{ Team, TeamMember }

class DeleteTeamMemberUseCase(val teamDao: TeamDAO) extends Logging {
  ...
  def execute(teamId: ObjectId, userId: ObjectId): Either[Errors, Team] = {
    validateTeam(teamId, userId).whenOk {
      ...
      val newTeamMembers = teamMembers.filterNot(_.user_id == userId)
  ...
  private def validateTeam(teamId: ObjectId, userId: ObjectId) = {
    ...
    val existsCheck = rule("user")(existingTeamMemberOpt != None, "This user is not a member of this team")
