...
class RegisterService(userDao: UserDAO, newUserAdder: NewUserAdder, invitationService: InvitationService) extends Logging {
  ...
  private def registerUserWithInvitation(login: String, emailLowerCase: String, password: String, invitationCode: String): Either[String, Unit] = {
    for {
      _ <- leftIfTrue(invitationCode.isEmpty, "To register in Codebrag you need registration link. Ask other Codebrag user to send you one.").right
      _ <- leftIfTrue(!invitationService.verify(invitationCode), "The registration link is not valid or was already used. Ask your friend your friend for another one.").right
