...
import com.softwaremill.codebrag.dao.{RepositoryHeadStore, UserDAO}
...
class RepoStatusServlet(val authenticator: Authenticator, repoDataProducer: RepoDataProducer, repoHeadStore: RepositoryHeadStore) extends JsonServletWithAuthentication with Logging {

  get("/") {
    repoDataProducer.createFromConfiguration() match {
      case Some(repoData) => {
        repoHeadStore.getRepoStatus(repoData.repositoryName) match {
