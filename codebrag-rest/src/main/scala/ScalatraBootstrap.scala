...
class ScalatraBootstrap extends LifeCycle with Beans with EventingConfiguration {
  ...
  override def init(context: ServletContext) {
    ...
    context.mount(new CommitsServlet(authenticator, commitListFinder, commentListFinder, commentActivity, commitReviewTaskDao, userReactionService, userDao, swagger, diffWithCommentsService, importerFactory), Prefix + CommitsServlet.MAPPING_PATH)
