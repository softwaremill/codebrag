...
class ScalaRegisterUITest extends CodebragUITest with AwaitilitySupport {
  ...
  test("register") {
    ...
    await atMost(60, TimeUnit.SECONDS) until {
      emailService.wasEmailSent(EMAIL, "SML Codebrag - registration confirmation for user " + LOGIN)
    }
  }
}
