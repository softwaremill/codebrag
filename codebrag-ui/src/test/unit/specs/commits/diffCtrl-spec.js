...
describe("DiffCtrl", function () {
    ...
    it('should not allow liking entire diff if user already liked it before', inject(function ($controller, authService) {
        // Given
        var userName = 'user name';
        authService.loggedInUser = {fullName: userName};
        scope.currentCommit = {
        ...
        // When
        scope.likeEntireDiff();

        // Then server not called
    }));

    it('should call server to like entire commit and save this like locally', inject(function ($controller, authService) {
        // Given
        authService.loggedInUser = {fullName: 'irrelevant full name'};
        scope.currentCommit = {
            data: {},
            info: {
                id: 12
            ...
            userAlreadyLikedCommit: function () {
                return false;
            },
        ...
        $controller('DiffCtrl', {$scope: scope});
        var returnedLikeData = {like: 'data'};
        $httpBackend.expectPOST('rest/commits/12/likes',
            '{"commitId":12}').respond(201, returnedLikeData);
        ...
        // Then
        expect(scope.currentCommit.data).toEqual(returnedLikeData)
    }));

    it('should not allow liking entire diff if current user is the author', inject(function ($controller, authService) {
        // Given
        var fullName = 'full name';
        authService.loggedInUser = {fullName: fullName};
        scope.currentCommit = {
            isUserAuthorOfCommit: function (name) {
